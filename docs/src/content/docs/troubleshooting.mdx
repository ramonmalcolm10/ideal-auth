...
### Different `purpose` between creation and verification

The verifier's required `purpose` is mixed into the HMAC signature, so a token only verifies against a verifier configured with the same purpose. If the token was created with `purpose: 'password-reset'` but you verify it with a verifier configured as `purpose: 'email-verification'`, `verifyToken()` returns `null`. This is by design — it prevents a token minted for one flow from being replayed in another. Make sure both sides use the exact same `purpose` string.
...
The user enters the code from their authenticator app, but `totp.verify()` returns `{ valid: false, counter: null }`.

### Code was already used (replay rejected)

`verify()` rejects any code at or before the persisted `lastUsedCounter`, so entering the same code twice within a period fails by design — the user must wait for the next code. Also make sure you persist the returned `counter` after each successful verification and pass the stored value back as `lastUsedCounter` (or `null` if the user has never verified a code).
