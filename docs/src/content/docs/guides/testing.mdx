...
The token verifier is stateless and deterministic (given the same secret and purpose), so it is straightforward to test. Use a test secret that is at least 32 characters long. The `purpose` field is required — creation throws without it:
...
describe('token verifier', () => {
  const verifier = createTokenVerifier({ secret: SECRET, purpose: 'test-tokens' });
  ...
  it('returns null with a different purpose', () => {
    // The purpose is mixed into the token signature, so a token from one
    // flow never verifies in another — even with the same secret.
    const token = verifier.createToken('user-1');
    const otherPurpose = createTokenVerifier({
      secret: SECRET,
      purpose: 'password-reset',
    });

    expect(otherPurpose.verifyToken(token)).toBeNull();
  });
...
describe('TOTP', () => {
  ...
  it('verifies a valid code', () => {
    ...
    expect(totp.verify(code, secret, null).valid).toBe(true);
  });
  ...
  it('accepts codes within the window (default +-1 step)', () => {
    ...
    expect(totp.verify(pastCode, secret, null).valid).toBe(true);
    expect(totp.verify(futureCode, secret, null).valid).toBe(true);
  });
  ...
  it('rejects a replayed code', () => {
    ...
    const first = totp.verify(code, secret, null);
    expect(first.valid).toBe(true);

    // Passing the persisted counter back rejects the same code
    const second = totp.verify(code, secret, first.counter);
