...
<Aside type="tip">
  Stateless sessions (like ideal-auth's encrypted cookies) cannot be revoked across domains without additional infrastructure. Each domain must clear its own session cookie. If you need instant revocation across all tenants, add a `sessions` table and check it on every request — see the [Session Invalidation](/ideal-auth/security/session-invalidation/) guide.
</Aside>
