...
const resetTokens = createTokenVerifier({
  ...
  purpose: 'password-reset', // tokens from other flows never verify here
...
3. **Verify the token and update the password**
   ...
   export async function resetPassword(req, res) {
     ...
     // Rate limit to prevent flooding with invalid tokens
     const ip = req.headers['x-forwarded-for'] ?? req.ip ?? '127.0.0.1';
     const { allowed } = await resetLimiter.attempt(`reset:${ip}`);
     if (!allowed) {
     ...
     // Update the password, record when it was changed,
     // and verify the email (they proved ownership by clicking the reset link)
     ...
     // Auto-login — the user just proved their identity
     const session = auth();
