...
3. **Verify the code and activate 2FA**
   ...
   export async function confirmTwoFactor(req, res) {
     ...
     // This is a fresh setup, so there is no previous counter — pass null.
     const { valid, counter } = totp.verify(code, secret, null);
     if (!valid) {
     ...
     await db.user.update({
       ...
       data: {
         ...
         // record the enrollment code's step so it can't be replayed at login
...
const pendingVerifier = createTokenVerifier({
  secret: process.env.IDEAL_AUTH_SECRET!,
  purpose: '2fa-pending', // tokens from other flows never verify here
...
export async function verifyTwoFactor(req, res) {
  ...
  // verify + the persisted counter rejects replayed codes —
  // a code that already logged someone in can't be used again within
...
`verify()` handles this: it returns the time step the code matched, and rejects codes at or before the `lastUsedCounter` you pass in. Persist the counter after each successful verification (as shown in Step 2 above) and every code becomes single-use:

```ts
const { valid, counter } = totp.verify(
  code,
  secret,
  user.totpLastUsedCounter, // null on first use
);
if (!valid) {
  return res.status(400).json({ error: 'Invalid or already used 2FA code' });
}

// counter is the step the code actually matched (may be the previous or
// next step within the drift window) — persist it, not the current time
await db.user.update({
  where: { id: user.id },
  data: { totpLastUsedCounter: counter },
});
...
CREATE TABLE users (
  ...
  totp_last_used_counter BIGINT         -- replay protection (verify)
);
```
