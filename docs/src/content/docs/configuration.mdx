...
**Cookie-backed (`sessionFields`):** The session cookie stores the user ID plus the declared fields. `user()` returns only `id` + the declared fields — everything else (including password) is excluded from the type.

Define the fields once as a `const` array and derive the type from it:
...
// Define fields once — `as const` preserves the literal types
...
const user = await auth().user();
// Type: Pick<DbUser, 'id' | 'email' | 'name' | 'role'> | null
// No password on the type — safe to pass to client
```
...
<Tabs>
  ...
  <TabItem label="Cookie-backed">
    ...
    export const auth = createAuth<User, (typeof sessionFields)[number]>({
      secret: process.env.IDEAL_AUTH_SECRET!,
      cookie: cookieBridge,
      sessionFields,
      resolveUserByCredentials: (creds) =>
  ...
  <TabItem label="Cookie-backed (no database)">
    ...
    const sessionFields = ['email', 'name', 'accessToken'] as const;
