...
Session cookies have a fixed expiry set at login time. To keep active users logged in, call `touch()` in your middleware to re-seal the cookie with a fresh expiry:

```ts
// In middleware or a route handler — where cookie writes are allowed
const session = auth();
...
`touch()` re-seals the session cookie with a new `iat`/`exp` using the same `maxAge` as the original session. If the user logged in with `remember: true` (30 days), `touch()` extends by 30 days. No database call, no user object needed. Does nothing if no valid session exists.

:::caution
Call `touch()` only in contexts where cookie writes are allowed (middleware, Route Handlers, Server Actions). Do not call it in Next.js Server Components — `cookies().set()` throws in read-only contexts. `check()`, `user()`, and `id()` are read-only and safe to call anywhere.
:::
