...
:::caution
Do not enable `autoTouch` globally in Next.js — `cookies().set()` throws in Server Components. Use per-request `auth({ autoTouch: true })` in middleware only.
:::
...
| `purpose` | `string` | — | **Required.** Label for the flow (e.g. `'password-reset'`, `'email-verification'`, `'magic-link'`). Mixed into the HMAC signature, so a token only verifies against a verifier with the same purpose — reuse one `IDEAL_AUTH_SECRET` across flows with distinct purposes. |
...
// Pass the persisted counter from the last successful verification,
// or null if the user has never verified a code.
const { valid, counter } = totp.verify('123456', secret, null);
// On success, persist `counter` (e.g. user.totpLastUsedCounter) and
// pass it back next time — codes at or before it are rejected.
```
...
| `verify(token, secret, lastUsedCounter)` | `{ valid: boolean; counter: number \| null }` | Verifies a TOTP code against the secret using timing-safe comparison, with built-in replay protection. Codes at or before `lastUsedCounter` are rejected. On success, persist the returned `counter` and pass it back on the next verification. Tokens are normalized: whitespace is stripped, and anything that is not exactly `digits` digits is rejected. |
...
<Tabs>
  <TabItem label="Database-backed">
    ...
    // Token verifier (password reset) — create one verifier per purpose,
