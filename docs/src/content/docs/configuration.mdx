...
| `resolveUser` | `(id: string) => Promise<TUser \| null>` | — | Looks up a user by ID. Called when rehydrating a session from an encrypted cookie. **Required** unless `sessionFields` is provided. |
| `sessionFields` | `(keyof TUser & string)[]` | — | Fields from the user object to store in the session cookie. When provided, `user()` returns the stored fields directly — no database call. **Required** unless `resolveUser` is provided. Cannot be used together with `resolveUser`. |
...
`createAuth` supports two mutually exclusive modes for how `user()` resolves the authenticated user. Provide exactly one of `resolveUser` or `sessionFields`.

**Database-backed (`resolveUser`):** The session cookie stores only the user ID. Every `user()` call invokes `resolveUser(id)` to fetch the full user from your database. Best when you need real-time data (e.g., role changes take effect immediately).
...
// user() hits the database on every call
const user = await auth().user();
```

**Cookie-backed (`sessionFields`):** The session cookie stores the user ID plus the declared fields. `user()` reads directly from the cookie — zero external calls. Best for performance, stateless architectures, or apps without a database.
...
// user() reads from the cookie — no database call
