...
The session payload contains `{ uid, iat, exp }` — the user ID, issued-at timestamp, and expiry timestamp (both in seconds). When using `sessionFields`, the payload also includes a `data` field containing the declared user fields (e.g., email, name, role). All fields — including `data` — are protected by iron-session's encryption and HMAC integrity. The cookie cannot be read or tampered with without the secret.

:::caution
With `sessionFields`, data is snapshotted at login time. If a user's role or permissions change server-side after login, the cookie retains the old values until the session expires or the user re-logs in. For authorization-critical fields (role, permissions, subscription tier), use `resolveUser` instead to fetch fresh data on every request.
:::
...
ideal-auth has exactly **one runtime dependency**:
...
[bcryptjs](https://github.com/nicolo-ribaudo/bcryptjs) is an **optional peer dependency** — only needed if you use the built-in `createHash()`. If you provide your own `HashInstance` (e.g., using `Bun.password` or argon2), bcryptjs is not installed or loaded. See the [configuration guide](/ideal-auth/configuration/#password-hashing) for details.
...
<Aside type="tip">
  Run `npm ls --production` or `bun pm ls` to verify the dependency tree yourself. You should see exactly one direct dependency (`iron-session`).
</Aside>
