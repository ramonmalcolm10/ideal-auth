...
- [ ] **Token verifier secret is 32+ characters.** Same requirement as `IDEAL_AUTH_SECRET`. You can use the same secret or a different one.
...
- [ ] **Distinct `purpose` per use case.** `createTokenVerifier` requires a `purpose`, and it is mixed into the token signature — a token only verifies against a verifier with the same purpose. This prevents a reset token from being used as a verification token and vice versa, even with a shared secret. One secret with distinct purposes is the recommended pattern; separate secrets per flow are no longer needed for separation.
...
- [ ] **TOTP secret is stored encrypted in the database.** The TOTP secret is the shared secret between your server and the user's authenticator app. If your database is compromised, an attacker with the plaintext secret can generate valid TOTP codes. Encrypt the secret before storing it:
  ...
  const { valid, counter } = totp.verify(code, decryptedSecret, user.totpLastUsedCounter);
  if (valid) {
    // Persist the matched counter for replay protection
    await db.user.update({ where: { id: userId }, data: { totpLastUsedCounter: counter } });
  }
  ```
...
- [ ] **TOTP replay protection is wired up.** A TOTP code is valid for the current time window (default: 30 seconds) plus the configured window tolerance (default: 1 step = 30 seconds on each side). `verify()` has built-in replay protection: codes at or before the `lastUsedCounter` you pass in are rejected. Persist the returned `counter` (a `totp_last_used_counter` column) after every successful verification and pass it back on the next call.
