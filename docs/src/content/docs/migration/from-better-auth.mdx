...
2. ### Install ideal-auth
   ...
   Add `IDEAL_AUTH_SECRET` to your environment:
...
7. ### Migrate 2FA
   ...
   **After (ideal-auth):**
   ...
   export async function verify2FA(userId: string, code: string) {
     ...
     // Pass the persisted counter of the last successful verification
     // (null for first use) — codes at or before it are rejected,
     // which gives you built-in replay protection.
     const { valid, counter } = totp.verify(code, secret, user.totpLastUsedCounter);

     if (!valid) {
       return { error: 'Invalid code.' };
     }

     // Persist the matched counter for the next verification
     await db.user.update({
       where: { id: userId },
   ...
   Better Auth stores 2FA state in the `accounts` table via its plugin system. With ideal-auth, you add `totpSecret`, `recoveryCodes`, `twoFactorEnabled`, and `totpLastUsedCounter` columns to your `users` table (or a separate table) and manage the state yourself.
...
|-------------------|--------|-------|
| `users` | **Keep** | Add `password` column if missing. Add `totpSecret`, `recoveryCodes`, `twoFactorEnabled`, `totpLastUsedCounter` if migrating 2FA. |
