...
Verifies a TOTP code against a secret, with built-in replay protection. `lastUsedCounter` is the persisted time-step counter of the last successfully verified code — pass `null` if the user has never verified one. Codes at or before `lastUsedCounter` are rejected, so a code can never be accepted twice.

On success, `counter` is the time step the code matched. You **must persist it** (e.g. a `totp_last_used_counter BIGINT` column on the users table) and pass it back on the next verification.

| Field | Type | Description |
|---|---|---|
| `valid` | `boolean` | Whether the code is valid within the configured time window and newer than `lastUsedCounter` |
| `counter` | `number \| null` | The matched time step on success, `null` on failure. Store it and pass it as `lastUsedCounter` next time |
...
// Persist the matched counter so this code can never be replayed
...
Tokens are normalized before verification: whitespace is stripped (so `"123 456"` works), and anything that is not exactly `digits` digits is rejected.
